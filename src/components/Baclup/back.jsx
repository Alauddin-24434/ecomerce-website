...
const CategoryPage = () => {
    ...
    const statikColorFilter = [
        "white", "black", "red", "golden"
    ] 
    const brands = Array.from(new Set(currentCategoryData.map(item => item.brand)));
    const colors = Array.from(new Set(currentCategoryData.flatMap(item => item.color)));
    ...
    const filteredData = currentCategoryData.filter(item => {
        ...
        const meetsColorFilter = colorFilter.length === 0 || item.color.some(color => colorFilter.includes(color.toLowerCase()));
